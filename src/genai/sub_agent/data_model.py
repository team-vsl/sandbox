...
from genai.models import Models


class DataModelAgentState(TypedDict):
    messages: Annotated[List, add_messages]
    data: Union[Models, None]
...
class DataModelAgent:
    ...
    def _load_system_prompt():
        with open("/var/vpbank/datacontracts/sandbox/data/system_prompt/model.txt", "r", encoding="utf-8") as fp:
            data = fp.read()
    ...
    def generate_object_node(self, state: DataModelAgentState) -> DataModelAgentState:
        ...
        parser = JsonOutputParser(pydantic_object=DataModelField)
    ...
    def invoke(self, user_query):
        state = DataModelAgentState(messages=[HumanMessage(content=user_query)])
