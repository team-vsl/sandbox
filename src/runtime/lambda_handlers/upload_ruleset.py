...
async def handler(event, context):
    ...
    try:
        ...
        if not ruleset_id or not content:
            raise Exps.AppException("Missing ruleset_id in path param or content in request body")

        # Upload ruleset
        upload_ruleset(ruleset_id, content)
        rb.set_status_code(200)
        rb.set_data({"message": "Ruleset uploaded successfully", "ruleset_id": ruleset_id})
        return rb.create_response()
    except Exps.AppException as error:
        logger.error("Error | [upload_ruleset]:", error)
        ...
        logger.error("Uknown error | [upload_ruleset]:", error, traceback.format_exc())
        error.message = (
            "There is an internal error in server Contact with Admin to get support."
        )
