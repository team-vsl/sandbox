...
from utils.glue import get_job_run
...
async def handler(event, context):
    ...
    try:
        ...
        logger.error(f"Error | [get_job_run]: {error}")
        ...
        logger.error(f"Uknown error | [get_job_run]: {error} {traceback.format_exc()}")
        error.message = (
            "There is an internal error in server Contact with Admin to get support."
        )
        return rb.create_error_response(Exps.UnknownException(str(error)))
    finally:
        logger.debug("End execution of [get_job_run]")
